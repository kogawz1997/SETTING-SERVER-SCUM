(function () {
  ...
  state.lootUi.spawnerGroupOpenMode = ['single', 'all', 'none'].includes(state.lootUi.spawnerGroupOpenMode) ? state.lootUi.spawnerGroupOpenMode : 'single';
  state.lootUi.flatRowSearch = state.lootUi.flatRowSearch || '';
  state.lootUi.flatRowOpenMode = ['single', 'all', 'none'].includes(state.lootUi.flatRowOpenMode) ? state.lootUi.flatRowOpenMode : 'single';
  ...
  function renderSimulationRollPlan(result) {
    const plan = result?.rollPlan || {};
    const unresolved = result?.unresolvedRefs || [];
    const detail = plan.kind === 'spawner'
      ? `${plan.groups || 0} ${uiText('กลุ่ม', 'groups')} · ${plan.resolvedRefs || 0}/${plan.refs || 0} ${uiText('refs ใช้ได้', 'resolved refs')} · ${uiText('จำนวน', 'quantity')} ${plan.quantityMin ?? '-'}-${plan.quantityMax ?? '-'}`
      : `${plan.entries || 0} ${uiText('รายการที่สุ่มได้', 'rollable entries')}`;
    const unresolvedRows = unresolved.slice(0, 5).map((entry) => `<div><code>${escapeHtml(entry.ref)}</code><span>${escapeHtml(`${uiText('กลุ่ม', 'group')} ${entry.groupIndex}`)}</span></div>`).join('');
    return `<div class="sim-card sim-roll-plan"><div class="section-head compact"><div><h4>${escapeHtml(uiText('แผนการสุ่มที่ใช้', 'Roll plan used'))}</h4><p class="muted">${escapeHtml(detail)}</p></div><span class="tag ${unresolved.length ? 'danger' : 'ok'}">${escapeHtml(unresolved.length ? `${unresolved.length} missing` : uiText('พร้อม', 'ready'))}</span></div>${unresolvedRows ? `<div class="warn-card sim-unresolved-list"><strong>${escapeHtml(uiText('ref ที่หาไม่เจอ', 'Unresolved refs'))}</strong>${unresolvedRows}</div>` : `<div class="success-card">${escapeHtml(uiText('ref ที่ใช้จำลอง resolve ได้ทั้งหมด', 'All simulated refs resolved.'))}</div>`}</div>`;
  }

  function renderSimulationExpectedCategories(result) {
    const rows = result?.expectedCategorySummary || [];
    const maxValue = Math.max(0.000001, ...rows.map((entry) => entry.expectedItemsPerRun || 0));
    return `<div class="sim-card sim-expected-category-card"><div class="section-head compact"><div><h4>${escapeHtml(uiText('สัดส่วนหมวดที่ควรออก', 'Expected category mix'))}</h4><p class="muted">${escapeHtml(uiText('คำนวณจาก probability / rarity ก่อนสุ่มจริง ใช้ดู balance เร็วกว่าอ่าน JSON', 'Computed from probability and rarity before random sampling. Faster for balance checks than reading JSON.'))}</p></div></div><div class="sim-category-list">${rows.map((entry) => `<div class="sim-category-row"><div><strong>${escapeHtml(catalogCategoryLabel(entry.category))}</strong><span>${escapeHtml(`${entry.expectedItemsPerRun} ${uiText('item/รอบ', 'items/run')} · ${Math.round((entry.share || 0) * 1000) / 10}%`)}</span></div><div class="sim-bar-track"><div class="sim-bar-fill" style="width:${Math.max(4, (entry.expectedItemsPerRun / maxValue) * 100)}%"></div></div></div>`).join('') || `<div class="muted builder-empty">${escapeHtml(uiText('ยังคำนวณ expected category ไม่ได้', 'Expected category mix is not available.'))}</div>`}</div></div>`;
  }
  ...
  function renderSimulationCompare(data) {
    ...
    const note = escapeHtml(uiText('Simulator นี้ใช้เทียบไฟล์ที่ save แล้วกับ draft ปัจจุบัน เพื่อดูทิศทางก่อนกด Save ไม่ใช่ runtime จริงของ SCUM แบบเป๊ะ', 'This simulator compares the saved file with the current draft to show direction before saving. It is not a perfect SCUM runtime clone.'));
    return `<div class="simulator-output"><div class="simulator-note">${note}</div><div class="mini-stat-grid"><div class="mini-stat"><span>${escapeHtml(uiText('รอบที่จำลอง', 'Runs'))}</span><strong>${escapeHtml(draft.count || saved.count || 0)}</strong></div><div class="mini-stat"><span>${escapeHtml(uiText('Avg item/run', 'Avg items/run'))}</span><strong>${escapeHtml(`${saved.averageItemsPerRun || 0} -> ${draft.averageItemsPerRun || 0}`)}</strong></div><div class="mini-stat"><span>${escapeHtml(uiText('ผลต่างเฉลี่ย', 'Average delta'))}</span><strong class="${averageDelta >= 0 ? 'positive' : 'negative'}">${escapeHtml(`${averageDelta >= 0 ? '+' : ''}${averageDelta}`)}</strong></div><div class="mini-stat"><span>${escapeHtml(uiText('ชนิด item', 'Distinct items'))}</span><strong>${escapeHtml(`${saved.distinctItems?.length || 0} -> ${draft.distinctItems?.length || 0} (${distinctDelta >= 0 ? '+' : ''}${distinctDelta})`)}</strong></div></div><div class="sim-grid">${renderSimulationTopList(saved, uiText('ไฟล์ที่ Save แล้ว', 'Saved file'))}${renderSimulationTopList(draft, uiText('Draft ตอนนี้', 'Current draft'))}${renderSimulationRollPlan(draft)}${renderSimulationExpectedCategories(draft)}${renderSimulationDeltas(data)}${renderSimulationSamples(draft)}</div></div>`;
  }
  ...
  function analyzerAdviceItems(overview = {}) {
    ...
    const lowCoverageSpawner = (overview.spawnerCoverage || []).find((entry) => entry.path && (Number(entry.coverage || 0) < 100 || Number(entry.refs || 0) === 0));
    const weaponHeavyNode = (overview.nodePowerScores || []).find((entry) => entry.path && Number(entry.categoryMix?.weapon || 0) > Number(entry.categoryMix?.ammo || 0));
    const medicalNode = (overview.nodePowerScores || []).find((entry) => entry.path && Number(entry.categoryMix?.medical || 0) > 0) || (overview.nodePowerScores || []).find((entry) => entry.path);
    const items = [];
    if (missingRefs.length) {
      items.push({
        ...
        view: 'graph',
        targetTerm: missingRefs[0]?.nodeName || missingRefs[0]?.spawner || '',
    ...
    if (Number(balance.spawnerCoverage || 0) < 90) {
      items.push({
        ...
        view: 'loot',
        targetPath: lowCoverageSpawner?.path || '',
        action: uiText('เปิด Loot Studio', 'Open Loot Studio')
      });
    }
    ...
    if (Number(balance.ammoToWeaponRatio || 0) < 0.75) {
      items.push({
        ...
        view: 'loot',
        targetPath: weaponHeavyNode?.path || '',
    ...
    if (!items.length && !(warnings || []).length) {
      items.push({
        ...
        view: 'loot',
        targetPath: (overview.nodePowerScores || [])[0]?.path || '',
        action: uiText('เปิด Loot Studio', 'Open Loot Studio')
      });
    }
  ...
  function renderAnalyzerAdvice(overview = {}) {
    ...
    host.innerHTML = `<div class="section-head compact"><div><h4>${escapeHtml(uiText('ควรทำต่อ', 'Next actions'))}</h4><p class="muted">${escapeHtml(uiText('แปลผลตัวเลขให้เป็นงานที่ควรไล่แก้ก่อน ไม่ต้องเดาเองจากกราฟทั้งหมด', 'Turns analyzer numbers into the first few tasks to check.'))}</p></div><span class="tag">${escapeHtml(String(actions.length))}</span></div><div class="analyzer-advice-grid">${actions.map((item) => `<article class="analyzer-advice-card ${escapeHtml(item.tone)}"><div><span class="tag ${escapeHtml(item.tone)}">${escapeHtml(item.tone)}</span><h5>${escapeHtml(item.title)}</h5><p>${escapeHtml(item.body)}</p>${item.targetPath ? `<small class="muted analyzer-target-path">${escapeHtml(item.targetPath)}</small>` : ''}</div><button type="button" class="ghost tiny" data-analyzer-advice-view="${escapeHtml(item.view)}" data-analyzer-target-path="${escapeHtml(item.targetPath || '')}" data-analyzer-target-term="${escapeHtml(item.targetTerm || '')}">${escapeHtml(item.action)}</button></article>`).join('')}</div>`;
    host.querySelectorAll('[data-analyzer-advice-view]').forEach((button) => {
      button.onclick = () => {
        ...
        if (view === 'graph' && targetTerm) {
          ...
          if ($('graph-filter')) $('graph-filter').value = targetTerm;
          if (typeof loadGraph === 'function') loadGraph().catch((error) => showToast(error.message || String(error), true));
          return;
        }
        if (view !== 'analyzer' && typeof setView === 'function') setView(view);
        if (view === 'analyzer') $('analyzer-unused')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      };
    });
  }
  ...
  function renderLootFieldCheatsheet(summary) {
    if (!state.lootUi.simpleMode) return '';
    const rows = summary?.kind === 'spawner'
      ? [
          [uiText('Probability', 'Probability'), uiText('โอกาสหรือ weight ของ preset นี้ ยิ่งสูงยิ่งถูกเลือกบ่อย', 'Chance/weight for this preset. Higher means it is picked more often.')],
          [uiText('Quantity Min/Max', 'Quantity Min/Max'), uiText('จำนวนของต่อรอบ ไม่ใช่โอกาสดรอป', 'How many items can spawn per roll, not the drop chance.')],
          [uiText('Node groups', 'Node groups'), uiText('ชุด ref ที่ spawner นี้จะไปสุ่มต่อ', 'Refs this spawner can roll next.')]
        ]
      : [
          [uiText('Class name', 'Class name'), uiText('ชื่อไอเท็มจริงของ SCUM ใช้ catalog ช่วยเลือกได้', 'Real SCUM item class. Use the catalog to pick it.')],
          [uiText('Probability / น้ำหนัก', 'Probability / weight'), uiText('ค่าสูงออกบ่อยกว่า ใช้ preset ด้านล่างเมื่อต้องปรับหลายแถว', 'Higher values appear more often. Use the presets below for selected rows.')],
          [uiText('Normalize', 'Normalize'), uiText('จัดสัดส่วนใหม่ให้แถวที่เลือกหรือทั้งรายการอ่านง่ายขึ้น', 'Rebalances selected rows or the full list into clearer proportions.')]
        ];
    return `<div class="loot-field-cheatsheet">${rows.map(([label, body]) => `<div><strong>${escapeHtml(label)}</strong><span>${escapeHtml(body)}</span></div>`).join('')}</div>`;
  }

  function renderProbabilityPresetButtons() {
    const presets = [
      ['trash', '0.01', uiText('ขยะ/หายากมาก', 'Trash / very rare')],
      ['rare', '0.05', uiText('Rare 0.05', 'Rare 0.05')],
      ['common', '0.25', uiText('Common 0.25', 'Common 0.25')],
      ['guaranteed', '1', uiText('ออกแน่ 1.00', 'Guaranteed 1.00')]
    ];
    return `<div class="probability-preset-grid">${presets.map(([id, value, label]) => `<button type="button" class="ghost tiny" data-prob-preset="${escapeHtml(id)}" data-prob-value="${escapeHtml(value)}">${escapeHtml(label)}</button>`).join('')}</div>`;
  }
  ...
  openLootFile = async function (path, options = {}) {
    ...
    state.lootUi.spawnerGroupOpenMode = 'single';
    state.lootUi.flatRowSearch = '';
    state.lootUi.flatRowOpenMode = 'single';
  ...
  renderFlatNodeBuilder = function (obj) {
    ...
    const rowOpenMode = ['single', 'all', 'none'].includes(state.lootUi.flatRowOpenMode) ? state.lootUi.flatRowOpenMode : 'single';
    ...
    const rowWorkbench = `<div id="flat-row-workbench" class="flat-row-workbench"><div class="flat-row-workbench-copy"><span id="flat-row-count" class="tag item">${escapeHtml(`${visibleRows.length}/${list.length} ${uiText('แถว', 'rows')}`)}</span><span class="muted">${escapeHtml(rowSearch ? uiText('กำลังกรองรายการไอเท็มอยู่', 'Item rows are filtered') : uiText('ใช้ปุ่มนี้เพื่อยุบแถวที่ยาว และเปิดเฉพาะตอนต้องแก้', 'Collapse long rows and expand only when you need to edit.'))}</span></div><div class="actions tight wrap"><button id="flat-row-clear-search" class="ghost tiny" ${rowSearch ? '' : 'disabled'}>${escapeHtml(uiText('ล้างค้นหา', 'Clear search'))}</button><button id="flat-row-expand-all" class="ghost tiny">${escapeHtml(uiText('เปิดแถวที่เห็น', 'Expand visible'))}</button><button id="flat-row-collapse-all" class="ghost tiny">${escapeHtml(uiText('ยุบแถวที่เห็น', 'Collapse visible'))}</button></div></div>`;
    const bulkPanel = `<div class="bulk-edit-panel"><div><strong>${escapeHtml(uiText('Bulk edit', 'Bulk edit'))}</strong><p class="muted">${escapeHtml(uiText('เลือกหลายแถวแล้วปรับ probability พร้อมกันได้ ไม่ต้องไล่แก้ทีละแถว', 'Select multiple rows and tune probability without editing one row at a time.'))}</p></div><div class="bulk-edit-grid"><div class="mini-stat"><span>${escapeHtml(uiText('เลือกอยู่', 'Selected'))}</span><strong>${escapeHtml(selectedRows.length)}</strong></div><label><span>${escapeHtml(uiText('ค่า probability', 'Probability value'))}</span><input id="bulk-prob-value" type="number" step="0.01" min="0" max="1" value="0.1" /></label>${renderProbabilityPresetButtons()}<div class="actions tight wrap"><button id="bulk-select-visible" class="ghost tiny">${escapeHtml(uiText('เลือกแถวที่เห็น', 'Select visible'))}</button><button id="bulk-clear-selection" class="ghost tiny">${escapeHtml(uiText('ล้างที่เลือก', 'Clear selection'))}</button><button id="bulk-set-prob" class="tiny">${escapeHtml(uiText('ตั้งค่า', 'Set value'))}</button><button id="bulk-boost-prob" class="ghost tiny">${escapeHtml(uiText('+10%', '+10%'))}</button><button id="bulk-reduce-prob" class="ghost tiny">${escapeHtml(uiText('-10%', '-10%'))}</button><button id="bulk-normalize-selected" class="ghost tiny">${escapeHtml(uiText('Normalize ที่เลือก', 'Normalize selected'))}</button><button id="bulk-delete-selected" class="danger-outline tiny">${escapeHtml(uiText('ลบที่เลือก', 'Delete selected'))}</button></div></div></div>`;
    ...
    const rows = visibleRows.map(({ entry, index }) => {
      ...
      const open = rowOpenMode === 'all' || (rowOpenMode !== 'none' && (index === 0 || (state.focusedLootField?.kind === 'flat' && state.focusedLootField.index === index)));
    ...
    if ($('flat-row-clear-search')) $('flat-row-clear-search').onclick = () => { state.lootUi.flatRowSearch = ''; renderVisualBuilder(); focusFieldSoon('#flat-row-search'); };
    if ($('flat-row-expand-all')) $('flat-row-expand-all').onclick = () => { state.lootUi.flatRowOpenMode = 'all'; renderVisualBuilder(); };
    if ($('flat-row-collapse-all')) $('flat-row-collapse-all').onclick = () => { state.lootUi.flatRowOpenMode = 'none'; renderVisualBuilder(); };
    ...
    document.querySelectorAll('[data-prob-preset]').forEach((button) => {
      button.onclick = () => {
        const value = Math.max(0, Math.min(1, Number(button.dataset.probValue || 0)));
        if ($('bulk-prob-value')) $('bulk-prob-value').value = String(value);
        applyBulkToSelected((entry) => setProb(entry, value));
      };
    });
  ...
  renderSpawnerBuilder = function (obj) {
    ...
    const groupOpenMode = ['single', 'all', 'none'].includes(state.lootUi.spawnerGroupOpenMode) ? state.lootUi.spawnerGroupOpenMode : 'single';
    ...
    const groupWorkbench = `<div id="spawner-group-workbench" class="flat-row-workbench spawner-group-workbench"><div class="flat-row-workbench-copy"><span id="spawner-group-count" class="tag spawner">${escapeHtml(`${visibleEntries.length}/${allEntries.length} ${uiText('กลุ่ม', 'groups')}`)}</span><span class="muted">${escapeHtml(groupSearch ? uiText('กำลังกรอง Node groups อยู่', 'Node groups are filtered') : uiText('ยุบกลุ่มที่ยาว แล้วเปิดเฉพาะกลุ่มที่กำลังแก้', 'Collapse long groups and open only the one you are editing.'))}</span></div><div class="actions tight wrap"><button id="spawner-group-clear-search" class="ghost tiny" ${groupSearch ? '' : 'disabled'}>${escapeHtml(uiText('ล้างค้นหา', 'Clear search'))}</button><button id="spawner-group-expand-all" class="ghost tiny">${escapeHtml(uiText('เปิดกลุ่มที่เห็น', 'Expand visible'))}</button><button id="spawner-group-collapse-all" class="ghost tiny">${escapeHtml(uiText('ยุบกลุ่มที่เห็น', 'Collapse visible'))}</button></div></div>`;
    const groups = visibleEntries.map(({ entry, index, ids }) => {
      const focused = state.focusedLootField?.kind === 'spawner' && state.focusedLootField.index === index;
      const open = groupOpenMode === 'all' || (groupOpenMode !== 'none' && (index === 0 || focused));
    ...
    if ($('spawner-group-clear-search')) $('spawner-group-clear-search').onclick = () => { state.lootUi.spawnerGroupSearch = ''; renderVisualBuilder(); focusFieldSoon('#spawner-group-search'); };
    if ($('spawner-group-expand-all')) $('spawner-group-expand-all').onclick = () => { state.lootUi.spawnerGroupOpenMode = 'all'; renderVisualBuilder(); };
    if ($('spawner-group-collapse-all')) $('spawner-group-collapse-all').onclick = () => { state.lootUi.spawnerGroupOpenMode = 'none'; renderVisualBuilder(); };
