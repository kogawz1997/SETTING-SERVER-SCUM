...
function renderItemCatalogSection(mode){
  ...
  const title = mode === 'tree' ? uiText('Item catalog สำหรับ tree', 'Item catalog for tree nodes') : uiText('Item catalog', 'Item catalog');
  const hint = mode === 'tree'
    ? uiText('เลือกไอเท็มจริงให้ leaf ที่กำลังแก้อยู่ หรือกดเพื่อเพิ่ม leaf ใหม่ได้ทันที', 'Pick a real item for the focused leaf, or add it as a new leaf immediately.')
    : uiText('เลือกชื่อไอเท็มจริงจาก catalog แทนการพิมพ์เดาเอง', 'Pick a real item instead of guessing the class name.');
  const toggleText = state.lootUi.itemCatalogOpen ? uiText('ซ่อน catalog', 'Hide catalog') : uiText('เปิด catalog', 'Show catalog');
  const options = (state.itemCatalog?.items || []).slice(0, 1200).map((item)=>`<option value="${escapeHtml(item.name)}"></option>`).join('');
  const categoryOptions = categories.map((entry)=>`<option value="${escapeHtml(entry.id)}"${entry.id === state.itemCatalogCategory ? ' selected' : ''}>${escapeHtml(`${entry.id} (${entry.count})`)}</option>`).join('');
  const resultCards = items.slice(0, 80).map((item)=>`<button type="button" class="result-card catalog-pick-card" data-catalog-pick="${escapeHtml(item.name)}"><strong>${escapeHtml(item.name)}</strong><div class="muted">${escapeHtml(`${item.category} · ${item.appearances} ${uiText('ครั้งที่พบ', 'hits')}`)}</div></button>`).join('');
  return `<div class="builder-section catalog-shell"><details class="catalog-panel" ${state.lootUi.itemCatalogOpen ? 'open' : ''}><summary class="section-head compact"><div><h4>${escapeHtml(title)}</h4><p class="muted">${escapeHtml(hint)}</p></div><div class="actions tight wrap"><span class="tag item">${escapeHtml(`${items.length}/${state.itemCatalog?.total || 0}`)}</span><button type="button" class="ghost tiny" data-item-catalog-toggle>${escapeHtml(toggleText)}</button></div></summary><div class="loot-inline-grid catalog-filter-grid"><label><span>${escapeHtml(uiText('ค้นหาไอเท็ม', 'Search items'))}</span><input id="item-catalog-search" value="${escapeHtml(state.itemCatalogSearch || '')}" placeholder="${escapeHtml(uiText('พิมพ์ชื่อไอเท็ม', 'Type an item name'))}" /></label><label><span>${escapeHtml(uiText('หมวด', 'Category'))}</span><select id="item-catalog-category"><option value="__all">${escapeHtml(uiText('ทุกหมวด', 'All categories'))}</option>${categoryOptions}</select></label></div><datalist id="loot-item-options">${options}</datalist><div class="catalog-result-grid">${resultCards || `<div class="muted builder-empty">${escapeHtml(uiText('ไม่พบไอเท็มที่ตรงกับคำค้นนี้', 'No items matched this search.'))}</div>`}</div></details></div>`;
}
...
function renderNodeRefCatalogSection(){
  ...
  const toggleText = state.lootUi.refCatalogOpen ? uiText('ซ่อน catalog', 'Hide catalog') : uiText('เปิด catalog', 'Show catalog');
  const nodeOptions = nodes.map((node)=>`<option value="${escapeHtml(node)}"${node === state.nodeRefNodeFilter ? ' selected' : ''}>${escapeHtml(node)}</option>`).join('');
  const refCards = refs.slice(0, 120).map((entry)=>`<button type="button" class="result-card catalog-pick-card" data-ref-pick="${escapeHtml(entry.ref)}"><strong>${escapeHtml(entry.label || entry.ref)}</strong><div class="muted">${escapeHtml(`${entry.node} · ${entry.kind}${entry.rarity ? ` · ${entry.rarity}` : ''}`)}</div><code>${escapeHtml(entry.ref)}</code></button>`).join('');
  return `<div class="builder-section catalog-shell"><details class="catalog-panel" ${state.lootUi.refCatalogOpen ? 'open' : ''}><summary class="section-head compact"><div><h4>${escapeHtml(uiText('Node ref catalog', 'Node ref catalog'))}</h4><p class="muted">${escapeHtml(uiText('เลือก tree ref จริงจากไฟล์ Nodes แทนการพิมพ์เองทีละบรรทัด', 'Use real tree refs instead of typing each one manually.'))}</p></div><div class="actions tight wrap"><span class="tag item">${escapeHtml(`${refs.length}/${state.nodeRefCatalog?.total || 0}`)}</span><button type="button" class="ghost tiny" data-ref-catalog-toggle>${escapeHtml(toggleText)}</button></div></summary><div class="loot-inline-grid catalog-filter-grid"><label><span>${escapeHtml(uiText('ค้นหา ref', 'Search refs'))}</span><input id="node-ref-search" value="${escapeHtml(state.nodeRefSearch || '')}" placeholder="${escapeHtml(uiText('พิมพ์ชื่อกิ่งหรือ ref', 'Type a branch name or ref'))}" /></label><label><span>${escapeHtml(uiText('ไฟล์ Node', 'Node file'))}</span><select id="node-ref-node-filter"><option value="__all">${escapeHtml(uiText('ทุก node', 'All nodes'))}</option>${nodeOptions}</select></label></div><div class="catalog-result-grid">${refCards || `<div class="muted builder-empty">${escapeHtml(uiText('ไม่พบ ref ที่ตรงกับคำค้นนี้', 'No refs matched this search.'))}</div>`}</div></details></div>`;
}
