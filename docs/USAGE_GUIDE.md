...
P2.13 local power-pack polish:

- ถ้าข้อความใน UI หรือเอกสารสำคัญหลุดเป็น question-mark blocks หรือ mojibake ระบบ `npm run release:check` จะ fail ทันที
- ใช้ `Visual Builder` เป็นโหมดหลัก, เปิด `Split View` เฉพาะตอนอยากเทียบ JSON, และใช้ `Raw JSON` เฉพาะตอนต้องแก้โครงสร้างเอง
- ถ้าไฟล์ลูทยาว ให้เริ่มจาก search/filter, workbench, item catalog และ context tabs แทนการไล่ scroll ทั้งหน้า
- Simulator มี note บอกชัดว่าใช้เทียบ draft/save เพื่อดูทิศทาง ไม่ใช่ตัวจำลอง runtime จริงแบบเป๊ะ

- Simulator now supports a seed and expected-rate summary for easier saved-vs-draft comparison.
- Graph focus can preview/apply spawner ref add/remove/replace changes through the safe backup flow, including drag-to-connect from spawner to node/ref.
...
ในหน้า Graph ใช้คลิก node เพื่อโฟกัส, ลากพื้นหลังเพื่อเลื่อน, ล้อเมาส์เพื่อ zoom และ filter เพื่อลดกราฟใหญ่ให้อ่านง่ายขึ้น ถ้าเปิด `ลากเชื่อม ref` สามารถลากจาก spawner ไปหา node/ref เพื่อเติมช่องแก้ ref แล้วค่อย preview ก่อน apply ได้
