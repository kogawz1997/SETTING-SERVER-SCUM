...
test('graph view has interactive focus and zoom controls', async ({ page }) => {
  ...
  const spawnerNode = page.locator('.graph-map-node.spawner').first();
  const targetGraphNode = page.locator('.graph-map-node.node').first();
  await expect(spawnerNode).toBeVisible({ timeout: 60000 });
  await expect(targetGraphNode).toBeVisible({ timeout: 60000 });
  const sourceBox = await spawnerNode.boundingBox();
  const targetBox = await targetGraphNode.boundingBox();
  if (!sourceBox || !targetBox) throw new Error('Graph drag nodes did not have a visible box');
  await page.mouse.move(sourceBox.x + sourceBox.width / 2, sourceBox.y + sourceBox.height / 2);
  await page.mouse.down();
  await page.mouse.move(targetBox.x + targetBox.width / 2, targetBox.y + targetBox.height / 2);
  await expect(page.locator('.graph-connect-preview-line')).toBeVisible();
  await expect(page.locator('.graph-connect-preview-label')).toContainText(/Release|ปล่อย/);
  await page.mouse.up();
  await expect(page.locator('.graph-staged-edit-summary')).toBeVisible();
