...
Current local-ready level: `P2.9`. Loot Studio now has an item row workbench for long loot lists, file scope filters, visible file counts, recent/pinned quick access, shareable links like `/loot-studio?file=Spawners/...`, and Analyzer/Search can open the exact loot file that needs work.
...
ตอนนี้อยู่ระดับ `P2.9` ใช้งานจริงแบบ local ได้แล้ว จุดหลักคือช่วยลดงานแก้ไฟล์ดิบ อ่านค่าลูทให้ง่ายขึ้น และช่วยกันพังก่อนเอา config ไปใช้กับเซิร์ฟจริง
